...
mod tests {
    ...
    fn c_ffi_sanity_tests(){
        let features1_ptr:*const *const f64 = vec![
            vec![100.0, 40.0, 22.0].as_ptr(),
            vec![211.0, 20.0, 2.0].as_ptr(),
            vec![32.0, 190.0, 150.0].as_ptr(),
            vec![2.0, 100.0, 100.0].as_ptr(),
        ].as_ptr();
        let weights1_ptr:*const f64 = vec![0.4, 0.3, 0.2, 0.1].as_ptr();

        let features2_ptr:*const *const f64 = vec![
            vec![0.0, 0.0, 0.0].as_ptr(),
            vec![50.0, 100.0, 80.0].as_ptr(),
            vec![255.0, 255.0, 255.0].as_ptr(),
        ].as_ptr();
        let weights2_ptr:*const f64 = vec![0.5, 0.3, 0.2].as_ptr();
